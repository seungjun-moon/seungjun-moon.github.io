...
![Alt text](/assets/img/posts/vae1/structure.png){: width="50%"",height="50%""}{:data-align="center"}

이 때, 우리는 $z$가 generator $g_{\theta}$를 거쳐서 생성 결과 $X$를 만들어낸다 할 수 있다. VAE(뿐만 아니라 AutoEncoder 역시)의 출력값이 아래와 같은 Gaussian 분포를 따른다고 가정하곤 한다.

$P(X|z;\theta)=\mathcal{N}(X|g_{\theta}(z), \sigma^{2}\dot I)$

풀어서 설명하면, $z$가 주어졌을 때 출력값 $X$의 확률 분포는 네트워크의 출력 값 $g_{\theta}(z)$을 평균으로 하고, $\sigma^{2}\dot I$을 분산으로 가지는 Gaussian 분포를 따른다는 뜻이다. 하지만 네트워크의 출력 값은 $g_{\theta}(z)$로 deterministic하게 정해지지 않는가? 물론 그렇지만, 이는 학습을 위한 전략이다. VAE는 학습 처음부터 (아마 나중에도) 주어진 학습 sample과 완전히 똑같은 출력 값을 만들진 못한다. 그러나 유사한 출력 값을 만들어나갈 수 있도록 적절한 loss를 디자인해줘야 하고, gradient descent를 통해 이를 줄여나가는 방식으로 학습이 진행되어야 한다. 그러나 네트워크의 출력 값이 deterministic하다고 해서, $P(X|z;\theta)$의 확률 분포를 Dirac delta 함수와 같이 만들어버리면, 우리는 Likelihood를 최대화하기 위한 gradient descent를 행할 수 없게 된다.
