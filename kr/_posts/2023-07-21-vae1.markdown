...
즉, 기존처럼 완전히 독립적인 분포 $p(z)에서부터 $z$를 sampling하는 것이 아닌, $X$에 따라 변하는 $z$의 분포에서 $z$를 sampling하는 것이 VAE의 핵심이다. 이러한 이상적인 sampling 함수를 만들기 위해 우리는 posterior, 즉 encoder $q_{\phi}$를 학습하게 된다.
